...
function createThumbnail( $fileSrc, $thumbDest, $thumb_width = 750, $thumb_height = 500 )
{
	...
	function thumb_all($name_orig,$name_new){
		...
		*/
		if(($originalWidth/$destWidth)<($originalHeight/$destHeight)){
				...
				$pos_x=0;
				$pos_y=-((($destWidth/$originalWidth)*$originalHeight-$destHeight)/2);
...
if (!empty($_FILES)) {
   ...
   thumb_all("../gal/".$newf_name, "../gal/thm_".$newf_name);
   $filearray = array();
